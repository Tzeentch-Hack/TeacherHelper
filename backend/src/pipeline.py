...
class Pipeline:
    ...
    def infer_first_step(self, text):
        analyzed_presentation_source, analyzed_presentation_eng, source_text = self.text_analyzer.analyze_presentation(text)
    ...
    def infer_whole(self, input_paths):
        ...
        for i, picture_description in enumerate(picture_descriptions, 1):
            img_path = f"../data/user/images/{i}.jpg"
        ...
        for d, s in zip(analyzed_presentation_source, img_paths):
            d["img_path"] = s
        self.get_presentation(analyzed_presentation_source, output_path="../data/user/presentations/presentation.pptx")
